...
export const Vortex = (props) => {
  ...
  const getThemeHue = () => {
    ...
    // Convert hex/rgb to HSL to get hue
    if (accentGreen.startsWith('#')) {
      const r = parseInt(accentGreen.slice(1, 3), 16) / 255;
      const g = parseInt(accentGreen.slice(3, 5), 16) / 255;
      const b = parseInt(accentGreen.slice(5, 7), 16) / 255;
    ...
    return props.baseHue || 220;
  };

  const baseHue = getThemeHue();
